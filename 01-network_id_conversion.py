...
mapper = mg.querymany(entrez_genes, scopes='entrezgene', fields='symbol,ensemblgene', species='human')

entrez_to_symbol = {
    d['query']: d['symbol']
    for d in mapper if 'symbol' in d
}
...
print("before conversion: %i\nafterconversion: %i" % (len(entrez_network), len(symbol_network)))
