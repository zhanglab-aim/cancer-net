...
from cancernet.pnet.config_path import PROSTATE_DATA_PATH
...
def prepare_design_matrix_crosstable():
    ...
    print(
        "number of mutations",
        total_numb_mutations,
        total_numb_mutations / (number_samples + 0.0),
    )
...
def prepare_response():
    ...
    df = pd.read_excel(
        join(data_dir, filename), sheet_name="Supplementary_Table3.txt", skiprows=2
    )
...
def prepare_cnv():
    ...
    df = pd.read_csv(
        join(data_dir, filename), sep="\t", low_memory=False, skiprows=1, index_col=0
    )
    df = df.T
    df = df.fillna(0.0)
