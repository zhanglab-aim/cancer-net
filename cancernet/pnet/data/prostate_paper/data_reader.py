...
from cancernet.pnet.config_path import *

data_path = DATA_PATH
processed_path = join(PROSTATE_DATA_PATH, "processed")

# use this one
gene_final_no_silent_no_intron = (
    "P1000_final_analysis_set_cross__no_silent_no_introns_not_from_the_paper.csv"
)
...
gene_important_mutations_only_plus_hotspots = (
    "P1000_final_analysis_set_cross_important_only_plus_hotspots.csv"
)
...
def load_data_type(
    ...
    if data_type == "mut_no_silent_no_intron":
        x, response, info, genes = load_data(
            gene_final_no_silent_no_intron, selected_genes
        )
        if mut_binary:
    ...
    if data_type == "mut_important":
        x, response, info, genes = load_data(
            gene_important_mutations_only, selected_genes
        )
        if mut_binary:
            logging.info("mut_binary = True")
            x[x > 1.0] = 1.0
    ...
    if data_type == "mut_important_plus_hotspots":
        x, response, info, genes = load_data(
            gene_important_mutations_only_plus_hotspots, selected_genes
        )
    ...
    if data_type == "truncating_mut":
        x, response, info, genes = load_data(
            gene_truncating_mutations_only, selected_genes
        )
        if mut_binary:
            logging.info("mut_binary = True")
            x[x > 1.0] = 1.0
...
def combine(
    ...
    if use_coding_genes_only:
        f = join(
            data_path,
            "genes/HUGO_genes/protein-coding_gene_with_coordinate_minimal.txt",
        )
...
def split_cnv(x_df):
    ...
    for g in genes:
        x_df[g, "CNA_deletion"] = x_df[g, "CNA_amplification"].replace(
            {-1.0: 0.5, -2.0: 1.0}
        )
        x_df[g, "CNA_amplification"] = x_df[g, "CNA_amplification"].replace(
            {1.0: 0.5, 2.0: 1.0}
        )
...
class ProstateDataPaper:
    def __init__(
        ...
        if type(data_type) == list:
            ...
            for t in data_type:
                x, y, rows, cols = load_data_type(
                    t, cnv_levels, cnv_filter_single_event, mut_binary, selected_genes
                )
                x_list.append(x), y_list.append(y), rows_list.append(
                    rows
                ), cols_list.append(cols)
            x, y, rows, cols = combine(
                x_list,
                y_list,
                rows_list,
                cols_list,
        ...
        if account_for_data_type is not None:
            ...
            for t in account_for_data_type:
                x_, y_, rows_, cols_ = load_data_type(
                    t, cnv_levels, cnv_filter_single_event, mut_binary, selected_genes
                )
                x_df = pd.DataFrame(x_, columns=cols_, index=rows_)
                x_list.append(x_df), y_list.append(y_), rows_list.append(
                    rows_
                ), cols_list.append(cols_)

            x_account_for = pd.concat(
                x_list, keys=account_for_data_type, join="inner", axis=1
            )
            x_all = pd.concat(
                [x_genomics, x_account_for],
                keys=["genomics", "account_for"],
                join="inner",
                axis=1,
            )
