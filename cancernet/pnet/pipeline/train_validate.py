...
from cancernet.pnet.utils.evaluate import evalualte
from cancernet.pnet.utils.plots import plot_confusion_matrix
...
def get_validation_primary(cols, cnv_split):
    ...
    valid_cnv = pd.read_csv(
        join(validation_data_dir, "PRAD/cnv_matrix.csv"), index_col=0
    )
    valid_mut = pd.read_csv(
        join(validation_data_dir, "PRAD/mut_matrix.csv"), index_col=0
    )

    genes = cols.get_level_values(0).unique()
    genes_df = pd.DataFrame(index=genes)

    valid_mut_df = genes_df.merge(
        valid_mut.T, how="left", left_index=True, right_index=True
    ).T
    valid_cnv_df = genes_df.merge(
        valid_cnv.T, how="left", left_index=True, right_index=True
    ).T
    ...
    if cnv_split:
        ...
        valid_cnv_ampl[valid_cnv_ampl <= 0.0] = 0.0
        valid_cnv_ampl[valid_cnv_ampl > 0.0] = 1.0

        valid_cnv_del = valid_cnv_df.copy()

        valid_cnv_del[valid_cnv_del >= 0.0] = 0.0
...
def get_validation_quigley(cols, cnv_split):
    ...
    valid_cnv = pd.read_csv(
        join(validation_data_dir, "/Quigley/cnv_design_matrix.csv"), index_col=0
    )
    valid_mut = pd.read_csv(
        join(validation_data_dir, "Quigley/2018_04_15_matrix_rna_tpm.txt"), index_col=0
    )
    ...
    valid_mut_df = genes_df.merge(
        valid_mut, how="left", left_index=True, right_index=True
    ).T
    valid_cnv_df = genes_df.merge(
        valid_cnv, how="left", left_index=True, right_index=True
    ).T
...
def get_validation_metastatic(cols, cnv_split):
    ...
    valid_cnv = pd.read_csv(
        join(validation_data_dir, "Met500/Met500_cnv.txt"), index_col=0, sep="\t"
    )
    valid_mut = pd.read_csv(
        join(validation_data_dir, "Met500/Met500_mut_matrix.csv"), index_col=0
    )

    valid_cnv = valid_cnv.T
    valid_cnv[valid_cnv > 1.0] = 1.0
    valid_cnv[valid_cnv < 0.0] = -1.0
    valid_mut.index = valid_mut.index.str.split(".", 1).str[0]

    genes = cols.get_level_values(0).unique()
    genes_df = pd.DataFrame(index=genes)

    valid_mut_df = common_samples.merge(
        valid_mut, how="inner", left_index=True, right_index=True
    )
    valid_cnv_df = common_samples.merge(
        valid_cnv, how="inner", left_index=True, right_index=True
    )

    valid_mut_df = genes_df.merge(
        valid_mut_df.T, how="left", left_index=True, right_index=True
    ).T
    valid_cnv_df = genes_df.merge(
        valid_cnv_df.T, how="left", left_index=True, right_index=True
    ).T
...
class TrainValidatePipeline:
    ...
    def save_prediction(
        self, info, y_pred, y_pred_score, y_test, model_name, training=False
    ):
    ...
    def run(self):
        ...
        (
            ...
            info_test_mets,
            cols_test_mets,
        ) = get_validation_metastatic(cols_train, cnv_split)
        (
            x_test_primary,
            y_test_primary,
            info_test_primary,
            cols_test_primary,
        ) = get_validation_primary(cols_train, cnv_split)
        ...
        print(
            x_test_mets.shape,
            y_test_mets.shape,
            info_test_mets.shape,
            cols_test_mets.shape,
        )
        print(
            x_test_primary.shape,
            y_test_primary.shape,
            info_test_primary.shape,
            cols_test_primary.shape,
        )
        ...
        if type(self.model_params) == list:
            for m in self.model_params:
                ...
                def predict(x_test, y_test, info_test, model_name, test_set_name):
                    ...
                    y_pred_test_scores = (
                        y_pred_test_scores1 + y_pred_test_scores2
                    ) / 2.0
                    ...
                    self.save_prediction(
                        info_test, y_pred_test, y_pred_test_scores, y_test, model_name
                    )
    ...
    def save_layer_outputs(
        self, x_train_layer_outputs, y_train, y_train_pred, x_test_layer_outputs, y_test
    ):
        fig = plt.figure(1, figsize=(10, 9))
        for i, (x_train, x_test) in enumerate(
            zip(x_train_layer_outputs[:-2], x_test_layer_outputs[:-2])
        ):
            ...
            X2d_xmin, X2d_xmax = np.min(X_embedded_train[:, 0]), np.max(
                X_embedded_train[:, 0]
            )
            X2d_ymin, X2d_ymax = np.min(X_embedded_train[:, 1]), np.max(
                X_embedded_train[:, 1]
            )
            xx, yy = np.meshgrid(
            ...
            plt.contourf(xx, yy, voronoiBackground, cmap=cmap, alpha=0.1)
    ...
    def save_score(self, score, model_name):
        ...
        with open(file_name, "w") as yaml_file:
            yaml_file.write(
                yaml.dump(
                    [self.data_params, self.model_params, self.pre_params, str(score)],
