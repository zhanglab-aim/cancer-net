...
class LeaveOneOutPipeline(OneSplitPipeline):
    ...
    def run(self):
        for data_params in self.data_params:
            ...
            for model_param in self.model_params:
                ...
                prediction_df = self.train_predict_crossvalidation(
                    m_param, X, y, info, cols, model_name
                )
    ...
    def save_prediction(
        self, info, y_pred, y_pred_score, y_test, fold_num, model_name, training=False
    ):
        if training:
            file_name = join(
                self.directory, model_name + "_traing_fold_" + str(fold_num) + ".csv"
            )
        else:
            file_name = join(
                self.directory, model_name + "_testing_fold_" + str(fold_num) + ".csv"
            )
    ...
    def save_score(
        self, data_params, model_params, scores, scores_mean, scores_std, model_name
    ):
                ...
                yaml.dump(
                    {
                        ...
                        "scores_mean": scores_mean.to_json(),
                        "scores_std": scores_std.to_json(),
                    },
