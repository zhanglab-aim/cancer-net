...
# from cancernet.pnet.model.model_utils import u.get_layers
import cancernet.pnet.model.model_utils as u
...
def predict(model, X, loss=None):
    ...
    if loss == "hinge":
        prediction = np.where(prediction_scores >= 0.0, 1.0, 0.0)
    else:
        prediction = np.where(prediction_scores >= 0.5, 1.0, 0.0)
...
def get_shap_scores_layer(
    model, X, layer_name, output_index=-1, method_name="deepexplainer"
):
    # local_smoothing ?
    # ranked_outputs
    def map2layer(model, x, layer_name):
        fetch = model.get_layer(layer_name).output
        feed_dict = dict(list(zip([model.layers[0].input], [x.copy()])))
    ...
    if method_name == "deepexplainer":
        ...
        shap_values, indexes = explainer.shap_values(
            map2layer(model, X, layer_name), ranked_outputs=2
        )
    elif method_name == "gradientexplainer":
        explainer = shap.GradientExplainer(
            (x, y), map2layer(model, X.copy(), layer_name), local_smoothing=2
        )
        shap_values, indexes = explainer.shap_values(
            map2layer(model, X, layer_name), ranked_outputs=2
        )
...
def get_shap_scores(
    model, X_train, y_train, target=-1, method_name="deepexplainer", detailed=False
):
    gradients_list = []
    gradients_list_sample_level = []
    i = 0
    for l in u.get_layers(model):
        if type(l) in [Sequential, Dropout, BatchNormalization]:
            continue
        if l.name.startswith("h") or l.name.startswith(
            "inputs"
        ):  # hidden layers (this is just a convention )
            if target is None:
                output = i
            else:
                output = target
            print(
                "layer # {}, layer name {},  output name {}".format(i, l.name, output)
            )
            i += 1
            # gradients = get_deep_explain_score_layer(model, X_train, l.name, output, method_name= method_name )
            gradients = get_shap_scores_layer(
                model, X_train, l.name, output, method_name=method_name
            )
...
def get_deep_explain_scores(
    model,
    X_train,
    y_train,
    target=-1,
    method_name="grad*input",
    detailed=False,
    **kwargs
):
    # gradients_list = []
    ...
    for l in u.get_layers(model):
        ...
        if l.name.startswith("h") or l.name.startswith(
            ...
            i += 1
            gradients = get_deep_explain_score_layer(
                model, X_train, l.name, output, method_name=method_name
            )
            if gradients.ndim > 1:
                # feature_weights = np.sum(np.abs(gradients), axis=-2)
...
def get_deep_explain_score_layer(
    model, X, layer_name, output_index=-1, method_name="grad*input"
):
    scores = None
...
def get_gradient_weights_with_repeated_output(model, X, y):
    ...
    # print 'layers', u.get_layers (model)
...
def get_weights_gradient_outcome(
    model,
    x_train,
    y_train,
    detailed=False,
    ...
    for l in layers:
        if l.name.startswith("h") or l.name.startswith(
            "inputs"
        ):  # hidden layers (this is just an ad hoc convention )
            ...
            print(
                "layer # {}, layer name {},  output name {}".format(
                    i, l.name, output.name
                )
            )
            i += 1
            print(i, l.name, output.name, output, l.get_output_at(0))
            gradients = get_gradeint(
                model,
                l.output,
                output,
                x_train,
