...
from cancernet.pnet.data.pathways.gmt_pathway import get_KEGG_map
from cancernet.pnet.model.builders.builders_utils import get_pnet
from cancernet.pnet.model.layers_custom import f1, Diagonal, SparseTF
from cancernet.pnet.model.model_utils import print_model, get_layers
...
def build_pnet(
    ...
    logging.info(
        "x shape {} , y shape {} info {} genes {}".format(
            x.shape, y.shape, info.shape, cols.shape
        )
    )
    ...
    outcome, decision_outcomes, feature_n = get_pnet(
        ins,
        features,
        genes,
        n_hidden_layers,
...
def build_pnet2(
    ...
    outcome, decision_outcomes, feature_n, pathway_map = get_pnet(
        ins,
        features=features,
        genes=genes,
...
def build_pnet2_account_for(
    ...
    assert (
        len(cols.levels) == 3
    ), "expect to have pandas dataframe with 3 levels [{'clinicla, 'genomics'}, genes, features] "
    ...
    clinical_outs = get_clinical_netowrk(
        ins_clinical, n_features, n_hids=[50, 1], activation=activation
    )
...
def build_dense(
    ...
    print(n)
    layer1 = Dense(
        units=int(n), activation=activation, W_regularizer=l2(w_reg), name="h0"
    )
    outcome = layer1(ins)
...
def build_pnet_KEGG(
    ...
    model.compile(optimizer=optimizer, loss=["binary_crossentropy"] * 3, metrics=[f1])
    # loss=['binary_crossentropy']*(n_hidden_layers +2))
    logging.info("done compiling")
