...
from cancernet.pnet.model.callbacks_custom import GradientCheckpoint, FixedEarlyStopping
from cancernet.pnet.model.model_utils import (
    get_layers,
    plot_history,
    get_coef_importance,
)
from cancernet.pnet.utils.logs import DebugFolder
...
class Model(BaseEstimator):
    ...
    def set_params(self, sk_params):
        ...
        if "reduce_lr_after_nepochs" in sk_params["fitting_params"]:
            ...
            self.reduce_lr_drop = sk_params["fitting_params"][
                "reduce_lr_after_nepochs"
            ]["drop"]
            self.reduce_lr_epochs_drop = sk_params["fitting_params"][
                "reduce_lr_after_nepochs"
            ]["epochs_drop"]
        else:
        ...
        pid = os.getpid()
        timeStamp = "_{0:%b}-{0:%d}_{0:%H}-{0:%M}-{0:%S}".format(
    ...
    def get_callbacks(self, X_train, y_train):
        ...
        if self.select_best_model:
            saving_callback = ModelCheckpoint(
                self.save_filename,
                monitor=self.monitor,
                verbose=1,
                save_best_only=True,
        ...
        if self.save_gradient:
            saving_gradient = GradientCheckpoint(
                self.save_filename,
    ...
    def get_validation_set(self, X_train, y_train, test_size=0.2):
        X_train1, X_validatioin, y_train_debug, y_validation_debug = train_test_split(
            X_train, y_train, test_size=test_size, stratify=y_train, random_state=422342
        )
    ...
    def fit(self, X_train, y_train, X_val=None, y_val=None):
        ...
        if self.class_weight == "auto":
            ...
            class_weights = class_weight.compute_class_weight(
                class_weight="balanced", classes=classes, y=y_train.ravel()
            )
        ...
        if self.debug:
            ...
            logging.info(
                "dividing training data into train and validation with split 80 to 20"
            )
            training_data, validation_data = self.get_validation_set(
        ...
        if hasattr(self, "feature_importance"):
            self.coef_ = self.get_coef_importance(
                X_train, y_train, target=-1, feature_importance=self.feature_importance
            )

        return self

    def get_coef_importance(
        self, X_train, y_train, target=-1, feature_importance="deepexplain_grad*input"
    ):

        coef_ = get_coef_importance(
            self.model, X_train, y_train, target, feature_importance, detailed=False
        )
        return coef_
    ...
    def predict(self, X_test):
        ...
        if self.loss == "mean_squared_error":
            ...
            prediction = np.where(prediction_scores >= std_th, 1.0, 0.0)
    ...
    def get_layer_output(self, layer_name, X):
        ...
        functor = K.function(
            inputs=[inp, K.learning_phase()], outputs=[layer.output]
        )  # evaluation function
        layer_outs = functor([X, 0.0])
        return layer_outs
    ...
    def get_layer_outputs(self, X):
        ...
        functor = K.function(
            inputs=[inp, K.learning_phase()], outputs=outputs
        )  # evaluation function
        layer_outs = functor([X, 0.0])
    ...
    def get_named_coef(self):
        ...
        coef = self.coef_
        coef_dfs = {}
        common_keys = set(coef.keys()).intersection(list(self.feature_names.keys()))
