...
class GNNExplainer(torch.nn.Module):
    ...
    def visualize_subgraph(
        self, node_idx, edge_index, edge_mask, y=None, threshold=None, **kwargs
    ):
        ...
        print("I am using visualization")
        # import pdb
        # pdb.set_trace()
        if threshold is not None:
            edge_mask = (edge_mask >= threshold).to(torch.float)
        edge_mask = (edge_mask >= 0.166).to(torch.float)
        ...
        node_kwargs["node_size"] = kwargs.get("node_size") or 100
    ...
    def __graphloss__(self, log_logits, pred_label):
    ...
    def explain_graph(self, data, **kwargs):
        ...
        # Get the initial prediction.
        with torch.no_grad():
            log_logits = self.model(data=data, **kwargs)
            pred_label = log_logits.argmax(dim=-1)
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            data_copy.x = h
            log_logits = self.model(data=data_copy, **kwargs)

            loss = self.__graphloss__(log_logits, pred_label)
            loss.backward()
            optimizer.step()
